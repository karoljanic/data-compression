...
FileInfo decode(std::ifstream &inputFile, std::ofstream &outputFile) {
    ...
    for (bytes_number_type byteNum = 0; byteNum < std::min(BLOCK_SIZE, allBytesNumber); byteNum++) {
    ...
    while (fileInfo.readBytesCounter < allBytesNumber) {
        ...
        calc_type scaledValue =
                ((value - lowerIntervalBound + 1) * fileInfo.charactersPartialSums[ALPHABET_SIZE] - 1) / intervalRange;
        ...
        upperIntervalBound = lowerIntervalBound +
                             (intervalRange * fileInfo.charactersPartialSums[character + 1]) /
                             fileInfo.charactersPartialSums[ALPHABET_SIZE] - 1;
...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cout << "Bad input!. Correct input is: ./decoder <encoded file> <output file>\n";
