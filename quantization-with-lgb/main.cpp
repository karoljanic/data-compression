...
double mse(const tga::Image &image1, const tga::Image &image2)
{
  ...
  for (uint16_t row = 0; row < image1.height; row++)
  {
    for (uint16_t col = 0; col < image1.width; col++)
    {
      mse +=
          std::pow(
              image1.colormap[row][col].blue - image2.colormap[row][col].blue,
              2) +
          std::pow(
              image1.colormap[row][col].green - image2.colormap[row][col].green,
              2) +
          std::pow(
              image1.colormap[row][col].red - image2.colormap[row][col].red, 2);
    }
  }
...
double snr(const tga::Image &image1, const tga::Image &image2)
{
  ...
  return std::log10(snr / mseValue / static_cast<double>(image1.height * image1.width));
}
...
void flattenColormap(const std::vector<std::vector<tga::Color>> &colormap,
...
tga::Color calculateAverageColor(const std::vector<tga::Color> &colors)
{
  ...
  return {static_cast<uint8_t>(static_cast<double>(blueSum) /
                               static_cast<double>(pixelsNumber)),
          static_cast<uint8_t>(static_cast<double>(greenSum) /
                               static_cast<double>(pixelsNumber)),
          static_cast<uint8_t>(static_cast<double>(redSum) /
                               static_cast<double>(pixelsNumber))};
}

double calculateAverageDistortion(const std::vector<tga::Color> &colors,
                                  const tga::Color &averageColor)
{
...
double calculateAverageDistortion(const std::vector<tga::Color> &colors,
                                  const std::vector<tga::Color> &codebook)
{
  double distortion{0.0};
  uint64_t pixelsNumber{0};
...
void splitCodebook(const std::vector<tga::Color> pixels,
                   std::vector<tga::Color> &codebook, double &distortion,
                   double epsilon, uint32_t maxIncreaseDistortionIterationsNumber)
{
  ...
  uint32_t increaseDistortionIterationsNumber{0};
  while (relativeError > epsilon && increaseDistortionIterationsNumber < maxIncreaseDistortionIterationsNumber)
  {
    std::vector<size_t> nearestCentroidsIndexes(pixels.size(), 0);
    std::vector<std::vector<size_t>> pixelsForNearestCentroids(
        newCodebook.size(), std::vector<size_t>());
    ...
    for (size_t pixelIndex = 0; pixelIndex < pixels.size(); pixelIndex++)
    {
      ...
      uint16_t nearestCentroidDistance{
          colorsDistance(pixels[pixelIndex], newCodebook[0])};

      for (size_t centroidIndex = 1; centroidIndex < newCodebook.size();
           centroidIndex++)
      {
        uint16_t distance{
            colorsDistance(pixels[pixelIndex], newCodebook[centroidIndex])};
    ...
    // update centroids
    for (uint32_t centroidIndex = 0; centroidIndex < newCodebook.size();
         centroidIndex++)
    {
      if (pixelsForNearestCentroids[centroidIndex].size() != 0)
      {
        std::vector<tga::Color> nearestPixels{};
        for (const auto &pixelIndex :
             pixelsForNearestCentroids[centroidIndex])
        {
    ...
    previousAverageDistortion =
        averageDistortion > 0 ? averageDistortion : distortion;
    averageDistortion = calculateAverageDistortion(pixels, closestCentroids);

    relativeError = std::abs((averageDistortion - previousAverageDistortion) /
                             averageDistortion);

    if (averageDistortion > previousAverageDistortion)
    {
      increaseDistortionIterationsNumber++;
    }
  }
...
void generateCodebook(const std::vector<tga::Color> &colors,
                      std::vector<tga::Color> &codebook, uint32_t colorsNumber,
                      double epsilon, uint32_t maxIncreaseDistortionIterationsNumber)
{
  ...
  {
    splitCodebook(colors, codebook, distortion, epsilon, maxIncreaseDistortionIterationsNumber);
  }
}

void quantify(const tga::Image &image, tga::Image &quantizedImage,
              uint32_t colorsNumber)
{
  ...
  generateCodebook(flattenedColormap, codebook, colorsNumber, 0.0001, 4);
  ...
  for (uint16_t row = 0; row < quantizedImage.height; row++)
  {
    ...
    {
      uint16_t nearestCentroidDistance{
          colorsDistance(quantizedImage.colormap[row][col], codebook[0])};
      size_t nearestCentroidIndex{0};
      for (size_t centroidIndex = 1; centroidIndex < codebook.size();
           centroidIndex++)
      {
        uint16_t distance{colorsDistance(quantizedImage.colormap[row][col],
                                         codebook[centroidIndex])};
        if (distance < nearestCentroidDistance)
        {
          nearestCentroidIndex = centroidIndex;
          nearestCentroidDistance = distance;
...
int main(int argc, char **argv)
{
  if (argc < 4)
  {
    std::cout << "Incorrect input. Enter input filename, output filename and "
                 "colors number description!\n";
    return -1;
  }
  ...
  {
    std::cout << "Total colors number: " << tga::countColors(image)
              << std::endl;
    ...
    std::cout << "Differs: " << tga::compareImages(image, quantizedImage)
              << " of " << (image.width * image.height) << std::endl;
