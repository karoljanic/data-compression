...
Statistics mse(const tga::Image& image1, const tga::Image& image2) {
  ...
  for (uint16_t row = 0; row < image1.height; row++) {
    for (uint16_t col = 0; col < image1.width; col++) {
      bMse += std::pow(
          image1.colormap[row][col].blue - image2.colormap[row][col].blue, 2);
      gMse += std::pow(
          image1.colormap[row][col].green - image2.colormap[row][col].green, 2);
      rMse += std::pow(
...
Statistics snr(const tga::Image& image1, const tga::Image& image2) {
  ...
  return Statistics{
      10 * std::log10(rSnr / mseValue.red /
                      static_cast<double>(image1.height * image1.width)),
      10 * std::log10(gSnr / mseValue.green /
                      static_cast<double>(image1.height * image1.width)),
      10 * std::log10(bSnr / mseValue.blue /
                      static_cast<double>(image1.height * image1.width)),
      10 * std::log10(totalSnr / mseValue.total /
                      static_cast<double>(image1.height * image1.width))};
}

int main(int argc, char* argv[]) {
  if (argc != 3) {
    std::cout << "Usage: " << argv[0] << " <original image> <decoded image>"
              << std::endl;
  }
