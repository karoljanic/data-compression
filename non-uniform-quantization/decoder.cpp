...
void decodeImage(std::ifstream& inputFile, std::string outputFilename) {
  ...
  for (size_t i = 0; i < bitsNumber; i++) {
    redChannelHigh.push_back(
        readNumberFromFile<int16_t, 16>(inputFile, buffer));
  }

  for (size_t i = 0; i < bitsNumber; i++) {
    greenChannelLow.push_back(
        readNumberFromFile<int16_t, 16>(inputFile, buffer));
  }

  for (size_t i = 0; i < bitsNumber; i++) {
    greenChannelHigh.push_back(
        readNumberFromFile<int16_t, 16>(inputFile, buffer));
  }

  for (size_t i = 0; i < bitsNumber; i++) {
    blueChannelLow.push_back(
        readNumberFromFile<int16_t, 16>(inputFile, buffer));
  }

  for (size_t i = 0; i < bitsNumber; i++) {
    blueChannelHigh.push_back(
        readNumberFromFile<int16_t, 16>(inputFile, buffer));
  }

  std::vector<int16_t> redChannel(2 * redChannelLow.size(), 0);
  int16_t redChannelLowAccumulator = clamp(redChannelLow[0]);
  redChannel[0] = redChannelLowAccumulator;
  for (size_t i = 1; i < redChannelLow.size(); i++) {
    redChannelLowAccumulator =
        clamp(redChannelLowAccumulator + redChannelLow[i]);
  ...
  int16_t blueChannelLowAccumulator = clamp(blueChannelLow[0]);
  blueChannel[0] = blueChannelLowAccumulator;
  for (size_t i = 1; i < blueChannelLow.size(); i++) {
    blueChannelLowAccumulator =
        clamp(blueChannelLowAccumulator + blueChannelLow[i]);
  ...
  int16_t greenChannelLowAccumulator = clamp(greenChannelLow[0]);
  greenChannel[0] = greenChannelLowAccumulator;
  for (size_t i = 1; i < greenChannelLow.size(); i++) {
    greenChannelLowAccumulator =
        clamp(greenChannelLowAccumulator + greenChannelLow[i]);
  ...
  for (size_t i = 0; i < image.height; i++) {
    for (size_t j = 0; j < image.width; j++) {
      image.colormap[i][j].red = clamp(redChannel[i * image.width + j]);
      image.colormap[i][j].green = clamp(greenChannel[i * image.width + j]);
      image.colormap[i][j].blue = clamp(blueChannel[i * image.width + j]);
    }
  }
